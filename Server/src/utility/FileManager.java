...
public class FileManager implements FileMethods, Serializable{
    ...
    public void parseCSVToCollection() {
        ...
        while(j < lines.length){
            ServerCollectionManager.group.put(Long.parseLong(lines[j]), new StudyGroup(Long.parseLong(lines[j + 1]), lines[j + 2], LocalDateTime.parse(lines[j + 3]), new Coordinates().convertStringToCoordinates(lines[j + 4]), Integer.parseInt(lines[j + 5]), Long.parseLong(lines[j + 6]), Integer.parseInt(lines[j + 7]), FormOfEducation.valueOf(lines[j + 8]), lines[9], new Person(lines[j + 10], Long.parseLong(lines[j + 11]), EyesColor.valueOf(lines[j + 12]), HairColor.valueOf(lines[j + 13]), Country.valueOf(lines[j + 14]), new Location(lines[j + 15], new Location().convertStringToCoordinatesX(lines[j + 16]), new Location().convertStringToCoordinatesY(lines[j + 17])))));
            j += 16;
        }
    }
