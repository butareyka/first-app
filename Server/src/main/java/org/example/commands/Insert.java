...
public class Insert extends ServerCommand implements Serializable {
    ...
    public Object executionForResponse(Object value, User user) throws IOException, SQLException {
        ...
        if (dataBaseManager.checkUser(user.getUserName())) {
            ...
            dataBaseManager.insertIntoStudyGroup(lastEntry.getValue().getGroupId(), user.getUserName(), lastEntry.getValue().getGroupName(), lastEntry.getValue().getCreationDate(), lastEntry.getValue().getStudentsCount(), lastEntry.getValue().getExpelledStudents(), lastEntry.getValue().getTransferredStudents(), lastEntry.getValue().getFormOfEducation(), lastEntry.getValue().getGroupId());
